...
test_that("log_compdens_conv_mix.normalnormal function works", {
    ...
    g <- normalmix(pi = pi_vals, mean = mean_seq, sd = sd_seq)
    ...
    for (index in 1:p) {
        errordist[[index]] <- normalmix(pi = pi_vals, mean = mean_seq[index],
                                              sd = sd_seq[index])
    }
    ...
    normal1 <- normalmix(pi = 1, mean = 0, sd = 0)
    normal2 <- normalmix(pi = 1, mean = 0, sd = 1)
    ...
    ## test point mass at zero where I know results
    normal1 <- normalmix(pi = 1, mean = 0, sd = 0)
    normal2 <- normalmix(pi = c(0.5, 0.5), mean = c(0, 0), sd = c(1, 2))
    mylike <- nnconv_dense(g = normal1, errordist = normal2, x = 0)
...
test_that("log_compdens_conv_mix.normaluni works", {
    ...
    errordist <- list()
    for (index in 1:p) {
        errordist[[index]] <- unimix(pi = pi_vals, a = a_seq, b = b_seq)
    }
    ...
    uni_part  <- unimix(pi = 1, a = -1, b = 1)
    norm_part <- normalmix(pi = 1, mean = 0, sd = 1)
    x <- 0
    ...
    uni_part  <- unimix(pi = 1, a = -1, b = 1)
    norm_part <- normalmix(pi = 1, mean = 0, sd = 0)
    x <- 0
...
test_that("unconv_dense works", {
    ...
    errordist <- list()
    for (index in 1:p) {
        errordist[[index]] <- normalmix(pi = pi_vals, mean = mean_seq,
                                              sd = sd_seq)
    }
    pi_vals <- rep(0.1, length = 5)
    a_seq   <- seq(-5, 0, length = 5)
    b_seq   <- seq(5, 0, length = 5)
    g <- unimix(pi = pi_vals, a = a_seq, b = b_seq)
...
test_that("uuconv_dense works", {
    ...
    g <- unimix(pi = pi_vals1, a = a_seq1, b = b_seq1)

    p <- 11
    betahat <- rnorm(p)
    errordist <- list()
    for (index in 1:p) {
        errordist[[index]] <- unimix(pi = pi_vals2, a = a_seq2, b = b_seq2)
    }
    ...
    g <- unimix(pi = 1, a = 0, b = 1)
    errordist <- unimix(pi = 1, a = 0, b = 1)
    ...
    ## Point mass at 0 example where I know results
    g  <- unimix(pi = 1, a = 0, b = 0)
    errordist <- unimix(pi = 1, a = -1, b = 1)
    x <- 0
